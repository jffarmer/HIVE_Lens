...
//import com.capstone.StoryOne.R;
...
public class ImageTargets extends Activity implements SensorEventListener {
	// ACCELEROMETER VARIABLE DECLARATION
	private SensorManager sensorManager;
	private static int newTarget = -1;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// ACCELEROMETER -- INITIALIZE SENSORMANAGER
	...
	public void onSensorChanged(SensorEvent event) {
		...
		// Determines Orientation of Device Based on Accelerometer Readings
		// Stores Orientation as an integer value of either 0 or 1
		if (event.sensor.getType() == Sensor.TYPE_ACCELEROMETER) {
			...
			if (x > -2.0f && y > 7.0f) {
				newTarget = 0;
//				Toast.makeText(this, "Target 0 chosen", Toast.LENGTH_SHORT).show();
				accelUpdate(newTarget);
			}
			else if (x > 7.0f && y > -2.0f) {
				newTarget = 1;
//				Toast.makeText(this, "Target 1 chosen", Toast.LENGTH_SHORT).show();
				accelUpdate(newTarget);
			}
			else if (x > -2.0f && y < -7.0f) {
				newTarget = 0;
//				Toast.makeText(this, "Target 0 chosen", Toast.LENGTH_SHORT).show();
				accelUpdate(newTarget);
			}
...
//		if (newTarget == 0){Toast.makeText(this, "Target 0 chosen", Toast.LENGTH_SHORT).show();}
//		else if (newTarget == 1){Toast.makeText(this, "Target 1 chosen", Toast.LENGTH_SHORT).show();}
//		
//		// Calls JNI Method so that it can update its texture id depending on
//		// current device orientation gardened from accelerometer
//		accelUpdate(newTarget);
	}
	...
	private void loadTextures() {
		...
		mTextures.add(Texture
				.loadTextureFromApk("solarpanels.png", getAssets()));
...
//        mp.release();
	...
	/** Native function to change texture based on Accelerometer. */
	private native void accelUpdate(int newTarget);
